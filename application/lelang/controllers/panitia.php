...
class Panitia extends CI_Controller {
    ...
    public function addJenisPos() {
        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") :
            ...
            $this->form_validation->set_rules("sertifikat_menang", "Sertifikat Menang", "required|callback__numeric_wcomma|callback__greater_than_wcomma[-1]");
            $this->form_validation->set_rules("sertifikat_kalah", "Sertifikat Kalah", "required|callback__numeric_wcomma|callback__greater_than_wcomma[-1]");
            $this->form_validation->set_rules("uang_menang", "Uang Menang", "required|callback__numeric_wcomma|callback__greater_than_wcomma[-1]");
            $this->form_validation->set_rules("uang_kalah", "Uang Kalah", "required|callback__numeric_wcomma|callback__greater_than_wcomma[-1]");
            //setting message
            $this->form_validation->set_message("required", "%s harus diisi");
            if ($this->form_validation->run() == TRUE):
